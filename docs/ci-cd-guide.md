...
> **Защита от публикации сломанного образа.** Job `docker-publish` зависит от release-please, но не от CI напрямую — оба идут параллельно. Чтобы избежать пуша сломанного образа, первый шаг `docker-publish` — `lewagon/wait-on-check-action`, который ждёт успеха CI-job `Сборка фронтенда` на том же коммите. Если CI упал — `docker-publish` падает на шаге ожидания, образ не уходит в Docker Hub.
