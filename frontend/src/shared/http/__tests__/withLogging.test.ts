...
describe('withLogging', () => {
  ...
  describe('sanitization', () => {
    ...
    it('masks multiple sensitive keys', async () => {
      const wrapped = withLogging(log)(inner);
      await wrapped('https://a.test/?password=abc&session=xyz&name=ok', { parse });

      const info: IHttpLogInfo = log.mock.calls[0][0];
      expect(info.url).not.toContain('abc');
      expect(info.url).not.toContain('xyz');
      expect(info.url).toContain('name=ok');
    });

    it('handles non-URL strings gracefully', async () => {
      const wrapped = withLogging(log)(inner);
      await wrapped('/relative/path', { parse });

      const info: IHttpLogInfo = log.mock.calls[0][0];
      expect(info.url).toBe('/relative/path');
    });
    ...
    it('uses default sanitizer when options object has no sanitize', async () => {
      const wrapped = withLogging({ log })(inner);
      await wrapped('https://api.test/?key=secret', { parse });

      const info: IHttpLogInfo = log.mock.calls[0][0];
      expect(info.url).not.toContain('secret');
    });
  });
});
